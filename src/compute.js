...
// ---- token-unlock calendar helpers -------------------------------------------------------------
// Unlocks are 24/7-market events with UTC timestamps — day bucketing is UTC CALENDAR DAYS, the
// deliberate counterpart to the earnings helpers' ET days (no cash session to anchor to).
function utcDayStr(ms) {
  const d = new Date(ms != null ? ms : Date.now());
  return d.getUTCFullYear() + "-" + String(d.getUTCMonth() + 1).padStart(2, "0") + "-" + String(d.getUTCDate()).padStart(2, "0");
}
function utcDayDiff(dateStr, nowMs) {
  if (typeof dateStr !== "string" || !/^\d{4}-\d{2}-\d{2}$/.test(dateStr)) return null;
  const a = Date.UTC(+dateStr.slice(0, 4), +dateStr.slice(5, 7) - 1, +dateStr.slice(8, 10));
  const t = utcDayStr(nowMs);
  const b = Date.UTC(+t.slice(0, 4), +t.slice(5, 7) - 1, +t.slice(8, 10));
  return Math.round((a - b) / DAY);
}
// DefiLlama /emissions summary -> the subset that IS our main-dex universe. Match order:
// explicit blocklist (memecoin symbol collisions), exact symbol, then the curated ticker->
// gecko_id alias table. Nothing fuzzy: an unmatched ticker is honestly absent. Returns
// [{ proto, gecko, ticker, coin, circ }] deduped one protocol per ticker (first match wins).
function matchLlamaProtocols(list, symToTk, aliasGeckoToTk, blocked) {
  const out = [], taken = new Set();
  if (!Array.isArray(list)) return out;
  for (const it of list) {
    if (!it || typeof it !== "object") continue;
    const sym = String(it.tSymbol || it.symbol || "").toUpperCase();
    const gecko = String(it.gecko_id || it.geckoId || "").toLowerCase();
    let m = null;
    if (sym && symToTk.has(sym)) m = symToTk.get(sym);
    if (!m && gecko && aliasGeckoToTk.has(gecko)) m = aliasGeckoToTk.get(gecko);
    if (!m || taken.has(m.ticker)) continue;
    if (blocked && blocked.has(m.ticker.toUpperCase())) continue;
    const proto = String(it.name || it.protocol || gecko || "").trim();
    if (!proto) continue;
    const circ = typeof it.circSupply === "number" && isFinite(it.circSupply) && it.circSupply > 0 ? it.circSupply : null;
    taken.add(m.ticker);
    out.push({ proto, gecko: gecko || null, ticker: m.ticker, coin: m.coin, circ });
  }
  return out;
}
// DefiLlama /emission/{protocol} -> forward CLIFF events inside the horizon, plus a linear-only
// flag. The endpoint has shipped both a raw object and a { body: "<json string>" } wrapper over
// time, and event lists have lived under .metadata.events and .events — accept all of them.
// noOfTokens can be a number or an array of per-allocation numbers (summed). Events without a
// usable timestamp or amount are dropped, never guessed. unlockType "linear" marks the drip.
function parseLlamaEvents(raw, nowMs, horizonDays) {
  let j = raw;
  try { if (j && typeof j.body === "string") j = JSON.parse(j.body); } catch (_) { return { events: [], linear: false, hadAny: false }; }
  if (j && typeof j === "string") { try { j = JSON.parse(j); } catch (_) { return { events: [], linear: false, hadAny: false }; } }
  const evs = (j && j.metadata && Array.isArray(j.metadata.events) && j.metadata.events)
    || (j && Array.isArray(j.events) && j.events) || [];
  const now = nowMs != null ? nowMs : Date.now();
  const hi = now + (horizonDays || 180) * DAY, lo = now - DAY;   // keep "earlier today" so TODAY never blanks mid-day
  const out = []; let sawLinear = false, hadAny = false;
  for (const e of evs) {
    if (!e || typeof e !== "object") continue;
    hadAny = true;
    const kind = String(e.unlockType || e.category || "").toLowerCase();
    if (kind === "linear") { sawLinear = true; continue; }
    let ts = typeof e.timestamp === "number" && isFinite(e.timestamp) ? e.timestamp : null;
    if (ts == null) continue;
    if (ts < 1e12) ts *= 1000;   // seconds vs ms — llama has shipped seconds
    if (ts < lo || ts > hi) continue;
    let amt = null;
    if (typeof e.noOfTokens === "number" && isFinite(e.noOfTokens)) amt = e.noOfTokens;
    else if (Array.isArray(e.noOfTokens)) { let s = 0, ok = false; for (const n of e.noOfTokens) if (typeof n === "number" && isFinite(n)) { s += n; ok = true; } if (ok) amt = s; }
    if (amt == null || amt <= 0) continue;
    out.push({ ts, d: utcDayStr(ts), amt, desc: typeof e.description === "string" ? e.description.slice(0, 140) : null });
  }
  out.sort((a, b) => a.ts - b.ts);
  return { events: out, linear: sawLinear && !out.length, hadAny };
}
module.exports.utcDayStr = utcDayStr;
module.exports.utcDayDiff = utcDayDiff;
module.exports.matchLlamaProtocols = matchLlamaProtocols;
module.exports.parseLlamaEvents = parseLlamaEvents;
