...
- **Drawdown tab** — best return since an anchor date against the max drawdown taken in the same
  window, one dot per market (top-left paid a lot and cost little; the dashed diagonal is where a
  name gave back as much as it ever made), with the same study as a sortable table and a CSV.
  Best = highest daily close over the anchor close with the live mark as today's point; drawdown =
  deepest close-to-close fall from any running peak (the daily feed carries no low, so wicks are not
  counted). Presets or any date in the last year; a name listed after the anchor is drawn hollow and
  marked *late*. A client-side study over `/api/daily`, no route of its own. Ships admin-only. The
  design mock is `docs/xyz-monitor-return-drawdown-mock.html`.
