...
const COLS=[
  ...
  {key:'momp', label:'MOM+', type:'num', def:'desc', tip:'CANDIDATE momentum, running head-to-head with the incumbent: same core, but the OI term is regime-qualified (OI building amplifies only when funding corroborates the score\u2019s side; falling OI is short-covering \u2014 mechanically different flow \u2014 and dampens at half strength) and a funding-crowding haircut applies when the crowd pays its own monthly extreme to sit on the score\u2019s side (\u00d70.8 \u2014 the exhaustion tax the \u25b4/\u25be flag points at). Hover a diverging cell for which terms fired. Adjudicated on daily forward rank IC in the Backtest tab\u2019s Score duel panel \u2014 promotion only on a locked verdict, not by eye.',
    td:r=>mompCell(r)},
  {key:'vol30', label:'Vol (ann)', type:'num', tip:'Annualized realized volatility. US/foreign session names: Yang-Zhang over the last 20 closed SESSION bars \u00d7\u221a252 \u2014 range-based (each session\u2019s true high and low plus its close-to-close move; on a 24/7 perp the session opens at the prior close, so the overnight term is zero). A window holding a bar without a true low falls back to the close-to-close \u03c3 of those 20 session returns; under 21 sessions, the hourly read. Crypto: hourly log returns over ~30 days \u00d7\u221a(24\u00b7365), the still-forming hour excluded. Hover a cell for the estimator. Carry divides by this.', td:r=>volCell(r)},
  {key:'adr', label:'Avg Range', type:'num', tip:'Average daily range: mean of each completed day\u2019s (high − low) / close, off the true hourly highs and lows. US names: per SESSION \u2014 weekends and holidays fold into the next session\u2019s range \u2014 over the last 5 sessions (21 when the 30d window is selected); crypto: calendar days, 7 (or 30). Reads as "on a typical session this moves X%."', td:r=>adrCell(r)},
  ...
  {key:'ddy', label:'vs YTD hi', type:'num', tip:'Distance below the year\u2019s highest DAILY HIGH (0% = making the YTD high now) \u2014 the candle\u2019s own high, the close on a bar without one. Names listed this year use their full-life high (which IS their YTD high). Crypto: 31d retention only reaches Jan 1 in January \u2014 outside that, an honest dash.', td:r=>ddyCell(r)},
  ...
  {key:'ma20', label:'MA 20', type:'num', tip:'20-session simple moving average of daily closes \u00b7 US names count US SESSIONS (weekends and exchange holidays fold into the next session\u2019s bar), crypto calendar days \u00b7 green when price is above it, red below \u00b7 fills in once daily history loads \u00b7 hover for distance', td:r=>maCell(r,'ma20',20)},
  {key:'ma50', label:'MA 50', type:'num', tip:'50-session simple moving average of daily closes \u00b7 US names count US SESSIONS (weekends and exchange holidays fold into the next session\u2019s bar), crypto calendar days \u00b7 green when price is above it, red below', td:r=>maCell(r,'ma50',50)},
  {key:'ma100', label:'MA 100', type:'num', tip:'100-session simple moving average of daily closes \u00b7 US names count US SESSIONS (weekends and exchange holidays fold into the next session\u2019s bar), crypto calendar days \u00b7 green when price is above it, red below \u00b7 crypto scope shows \u2014 (~90d on the wire)', td:r=>maCell(r,'ma100',100)},
  {key:'ma200', label:'MA 200', type:'num', tip:'200-session simple moving average of daily closes (\u2248 10 months on a US name, not 200 UTC days) \u00b7 US names count US SESSIONS (weekends and exchange holidays fold into the next session\u2019s bar), crypto calendar days \u00b7 green when price is above it, red below \u00b7 crypto scope shows \u2014 (~90d on the wire)', td:r=>maCell(r,'ma200',200)},
