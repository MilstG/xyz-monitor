...
<div class="hlp-h">What it is not</div><p>Tick-exact: the lows are daily candle lows, and a bar's own high never counts against its own low (the order inside a day is unknown), so a same-day spike-and-flush is under-read. Older history restored without a low reads on the close there, and the caption counts those names. Crypto history on the wire is about 90 days, so an older anchor starts every coin late and the header says how many.</p>`,
  ematouch:`<div class="hlp-h">What it is</div><p>Every name's <b>50 and 200 EMA</b> on the <b>4H</b> and <b>1D</b> candles: which are touching a line right now, and which are about to. The line is the EMA over closed candles carried to the live mark, which is what a chart draws while the candle forms. 1D is the session series on a US name (weekends and holidays fold into the next session), calendar days on crypto.</p><div class="hlp-h">Near a line</div><p>Every line within 1.5\u03c3 of the mark, nearest first. \u03c3 is that rung's own bar-return stdev (the unit the EMA200 alerts use), so 0.5\u03c3 is equally close on BTC and on GOLD; the % is beside it. The shaded band is the near zone (0.5\u03c3). <b>stacked</b> = the 50 and the 200 sit within half a sigma of each other, so one touch tests both.</p><div class="hlp-h">Touch cards</div><p>A touch (the forming candle reaches the line) opens a card, and at that candle's close it resolves: <b>Support held</b> / <b>Resistance held</b> (closed back on the side it came from) or <b>Broke down</b> / <b>Reclaimed</b> (closed through). Each card says in one line what it means and what would invalidate it, e.g. <i>"Bullish while above 148.05. A 4H close below it invalidates."</i></p><div class="hlp-h">How long a card matters</div><p>Cards age in candles of their own timeframe, not hours. <b>Fresh</b> = resolved within the last 3 bars (4H) or 2 sessions (1D): the actionable window. <b>Fading</b> = older (dimmed): context, not a setup. After 6 bars (4H) or 5 sessions (1D) the card leaves. <b>✓ worked</b> = price has gone at least 1σ from the line the way the card expects. <b>✕ failed</b> = a later close went back through the line; the card fades at once. <i>since</i> is where price is now, in σ from the line, in the expected direction. The big number on the left of every card is <b>how long ago it touched</b> (12m, 3h 20m, 1d 6h), with the clock time under it. <b>NEW</b> marks touches since your last visit. The feed is grouped into <b>Touching now</b> (candle still open, soonest close first), <b>Fresh</b> and <b>Fading</b>, newest first in each; <i>View: newest first</i> drops the groups.</p><div class="hlp-h">Cooldown</div><p>One card per line per candle. After a card the line has to <b>re-arm</b> (a close at least 1σ clear of it); touches before that fold into the same card as retouches.</p><div class="hlp-h">Alerts</div><p>Four classes, each picked separately in the bell's delivery panel (operator-only while this tab soaks): <b>ma200</b> and <b>ma50</b> are close-confirmed reclaim / breakdown / bullish and bearish retests; <b>touch200</b> and <b>touch50</b> are the intrabar touches, opt-in. Nothing is announced in the first ~6 minutes after a deploy: every line seeds silently first.</p>`,
