...
Object.assign(HELP,{
  ...
  drawdown:`<div class="hlp-h">What it is</div><p>One dot per market, two numbers that every other tab shows one of but never side by side. <b>Up</b> is the <b>best return</b> the name printed since the anchor: its highest daily close over the anchor close (the live mark counts as today's point, so a name making its high right now reads that way). <b>Right</b> is the <b>max drawdown</b> it took in the same window: the deepest close-to-close fall from any running peak. Top-left paid a lot and cost little; bottom-right dug a hole and never paid.</p>
<div class="hlp-h">The diagonal</div><p>The dashed line is where best return equals drawdown. Above it a name made more than it ever gave back; below it the worst stretch was bigger than the best one. The <b>Best / DD</b> column is the same read as a number.</p>
<div class="hlp-h">Anchor</div><p>The presets count back from today's UTC midnight (YTD is January 1); the date box takes any day in the last year. Bars are UTC days, so a weekend anchor starts on the Monday. A name whose first close is well after the anchor is drawn hollow and marked <b>late</b>: it starts at its own first close, which is not the same race.</p>
<div class="hlp-h">What it is not</div><p>Closes only — the daily feed carries no intraday low, so a wick below the close is not a drawdown here. Colour is only the sign of <b>now</b> (green above the anchor, red below); the axes carry the study. Crypto history on the wire is about 90 days, so an older anchor starts every coin late and the header says how many.</p>`,
