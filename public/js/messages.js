...
async function dmSend(){
  ...
  const ov=(!dmState.editing&&dmState.callOverride&&dmState.callOverride.text===text)?{side:dmState.callOverride.side||null,days:dmState.callOverride.days||null}:null;
...
function dmStampPreview(text){
  ...
  const ov=dmState.callOverride&&dmState.callOverride.text===String(text)?dmState.callOverride:null;
  ...
  const vague=!ov&&(!read.sideWord||!read.horizonWord)&&words>=4;
  ...
  const ask=vague&&dmAskAllowed()?' <button type="button" class="dm-tool" id="dm-callask" title="Ask the model what this message means \u2014 you apply the reading or ignore it; nothing posts on its say-so. Spends one ask.">ask AI what I mean</button>':'';
  ...
  pv.hidden=false; pv.innerHTML='will stamp <b>'+esc(r.ticker)+'</b> at <b>'+fmtPrice(r.px)+'</b> as <b class="'+(side==='short'?'neg':'pos')+'">'+side+'</b> \u00b7 <b>'+days+'d</b> '+why+ask+drop+(dmState.callAsking?' <span class="sec">asking\u2026</span>':'')
    +(dmState.callProposal&&dmState.callProposal.text===String(text)&&!ov
...
async function dmCallAsk(){
  ...
  const text=ta.value; const m=String(text).match(/\$([A-Za-z][A-Za-z0-9.\-]{0,9})/); if(!m) return;
  dmState.callAsking=true; dmStampPreview(text);
  let d;
  ...
  if(!d||!d.ok){ dmState.callProposal=null; dmStampPreview(ta.value); dmLocal('\u2717 could not read the call \u2014 '+esc((d&&d.error)||'no answer'),'err'); return; }
...
function dmWire(){
  ...
  host.addEventListener('click',(e)=>{
    ...
    if(e.target.id==='dm-callapply'){ const p=dmState.callProposal, ta=el('dm-input'); if(p&&ta&&p.text===ta.value){ dmState.callOverride={text:p.text,side:p.side,days:p.days,why:p.why}; dmState.callProposal=null; dmStampPreview(ta.value); } return; }
