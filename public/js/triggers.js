...
const ALERT_CHANNELS={
  ...
  ma50:{toast:true,bell:true,tg:true,push:false,note:'operator-only while EMA Touch soaks'},
  touch200:{toast:false,bell:true,tg:true,push:false,note:'intrabar; only enters the stream while someone opts in'},
  touch50:{toast:false,bell:true,tg:true,push:false,note:'intrabar; only enters the stream while someone opts in'},
...
function buildPushSection(){
  ...
  const recips=mineOnly.length? mineOnly.map(r=>{
    ...
    const CTIP={
      ...
      ma50:'EMA50 events, close-confirmed on the rung\u2019s own candle (H4 + D1): the same four shapes as ma200 (reclaim, breakdown, bullish/bearish retests) on the 50. Its own class so the 50 can be off while the 200 stays on. Operator-only while the EMA Touch tab soaks',
      touch200:'intrabar: the forming H4 or D1 candle reached the 200 EMA. One per touch episode \u2014 the line re-arms only after a close at least 1\u03c3 clear of it. Held vs through is decided at the close (ma200 carries that). Operator-only while the EMA Touch tab soaks',
      touch50:'intrabar: the forming H4 or D1 candle reached the 50 EMA. One per touch episode, same re-arm rule as touch200 \u2014 the noisiest EMA class by far on H4. Operator-only while the EMA Touch tab soaks',
