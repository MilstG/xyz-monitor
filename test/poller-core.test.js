...
test("study wiring 2026.09.20: hourly funding nets the daily base rates, the 5m archive resolves the anchors, ondrift carries the overnight split", async () => {
  ...
  assert.ok(pol.includes("detectPead(prints, r.dailyRaw, r.px, sd30, r.hourlyRaw, now, { off: sessOffOf(r) })"), "PEAD reads earnReactWindow's cash closes off the hourly spine, the session calendar for its fallback (-108)");
