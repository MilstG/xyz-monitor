...
test("-109 /api/usage: signed-out is a no-op; a member's beacon is validated, clamped to wall time, rate-limited and device-classed", async () => {
  ...
  assert.equal(anon.statusCode, 204, "signed out: acknowledged (build -117: counted as anonymous totals under uid '-1', never a member — test/usage-117.test.js)");
...
test("-109 admin usage: operator-only, cached on the flush generation, the drill-in writes view-usage to the read log", async () => {
  ...
  assert.equal(d.r, 30, "range max = the 30-day retention"); assert.equal(d.publicOn, true, "(build -117) public counting defaults on"); assert.equal(d.keepDays, 30);
