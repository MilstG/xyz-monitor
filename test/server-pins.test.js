...
test("perf batch 2026.07.21-08: getFunding memo, bucketsFor memo, gzip+dataTs wiring are all present", () => {
  ...
  assert.ok(app.includes("for(const r of state.rows.values()) if(r.feat) r.candleTs=now;"), "an unchanged snapshot still refreshes the feature stamp — the stale dimming reads a quiet tape as a stalled poller otherwise");
